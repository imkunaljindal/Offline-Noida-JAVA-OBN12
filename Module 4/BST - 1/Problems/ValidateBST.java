...
class Solution {
    static boolean helper(Node node, int lowerBound, int upperBound) {
        if(node==null) return true;
        if(node.data < lowerBound || node.data > upperBound) return false;

        boolean lst = helper(node.left,lowerBound,node.data-1);
        boolean rst = helper(node.right,node.data+1,upperBound);
        return lst && rst;
    }
