...
class Solution {
    ...
    public int maxOfTree(Node root) {
        ...
        return Math.max(Math.max(lstMax,rstMax),root.data);
    }

    public int heightOfTree(Node root) {
        if(root==null) return 0;

        int lstMaxheight = heightOfTree(root.left);
        int rstMaxHeight = heightOfTree(root.right);
        return Math.max(lstMaxheight,rstMaxHeight) + 1;
    }
}